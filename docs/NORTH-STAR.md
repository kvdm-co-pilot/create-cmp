...
| **0.5 — the console into the harness** | Provider interface named from the existing tool contracts; console + MCP server into the harness; section types formalised; tool listing profile-driven; `cmp` providers extracted; neutral skills moved | `node scripts/stage05-gate.mjs` — a recorded baseline for "exactly as today", an honesty floor a section can FAIL, sections declared rather than assumed, no Compose furniture off-Compose, and the console inside the harness package | Stage 0 exit; before Stage 1 so the package boundary is drawn with the console inside | **4/5.** Sections are declarable and the honesty floor holds; the console still lives in `inspector/mcp`, which Stage 1 would not ship |
