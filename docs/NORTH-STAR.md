...
| **1 — distribution** | The package split (PACKAGE-SPLIT Phases C–E): `prooflane-harness`, `prooflane-cli`, `prooflane-profile-cmp`, `prooflane-studio-cmp`; the lock as a stamper-written manifest; resolve local → node_modules → registry → git URL, fetch at init/new only | `node scripts/stage1-gate.mjs` — a foreign backend installs the harness with NO create-cmp present and gets a command; that command installs a lane reaching a green verify; a core fix published at a higher version reaches the adopter by ONE command; and **the fix arrives IN THE TREE** — the vendored bytes and the lock's per-file digests move, not merely a number in a manifest | Stage 0.5 exit **and** ADR-0007 signed (O4) **and** the Stage 1 ADR (O2) | **2/4 — A and B met 2026-09-08.** The harness has its own binary: `prooflane init` / `prooflane relock` (`packages/harness/bin/`), one implementation shared with `create-cmp harness init`, which now delegates. A Go repo with no create-cmp installs the tarball, runs `prooflane init`, and reaches `verify lane: PASS`. The move made three couplings pay for themselves: the installer's detectors are derived from the profiles the package SHIPS, the not-vendored tool list is derived (`install/portability.mjs` — the profile declares `tools`, the import closure checks it) instead of being hand-mirrored in two packages, and the receipt schema now ships IN the package, which a registry install had silently gone without. **C and D remain**, and honestly: no `upgrade` exists, so a core fix cannot yet reach an installed lane by one command. **Criterion amended 2026-09-08:** it read "a core fix reaches it by version bump", which ADR-0008 contradicts — under it the harness is always vendored, so a fix arrives as RESOLVE → VENDOR → RE-LOCK by one command, never as a package manager moving a number. The ADR flagged the contradiction itself and asked for the wording to be read that way or amended; amending it is what made the criterion evaluable |
