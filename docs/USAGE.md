...
| `smoke` | 7 | Every pure-Node gate and **no Gradle** — the smallest end-to-end lane through the real runner, receipt and journal. Seconds. Proves the *framework* returns, both ways; never the change — its receipt is refused as done-evidence. Driven by `node scripts/framework-check.mjs` (GATE-RULES Rule 0). |
