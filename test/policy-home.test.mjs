...
/** The homes may quote the episode — "printed `fleet L2 REQUIRED`" is how they explain it. */
const HOMES = new Set(["docs/NORTH-STAR.md", "docs/GATE-RULES.md"]);
...
function trackedDocs() {
  return execSync("git ls-files -- '*.md' docs agents skills .claude template/CLAUDE.md", { cwd: ROOT, encoding: "utf8" })
    .split("\n")
    .filter((f) => f.endsWith(".md") && !HOMES.has(f));
}
...
test("no tracked document outside the home states the device tier's cadence", () => {
  const hits = [];
  ...
  assert.deepEqual(hits, [], `restatements outside NORTH-STAR §7 / GATE-RULES Rule 4 — cite the home instead:\n${hits.join("\n")}`);
});
