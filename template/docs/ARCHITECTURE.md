...
<!-- <<< cmp:feature room -->
<!-- >>> cmp:feature !room -->
| Reliability (offline) | Network drops mid-session; a remote read fails as a typed `DomainError` and the screen shows its error state — the app keeps no on-device copy to fall back on | `[advisory]` — out of scope by decision: no on-device store ships (§7 Persistence, and the persistence ADR in §8) |
<!-- <<< cmp:feature !room -->
...
<!-- <<< cmp:feature room -->
<!-- >>> cmp:feature !room -->
that needs it. Each `actual` wraps that platform's own connectivity or automation API —
open the file for the concrete mechanism:
<!-- <<< cmp:feature !room -->
...
<!-- <<< cmp:feature room -->
<!-- >>> cmp:feature !room -->
│ data           remote/ (repository implementations)                │
<!-- <<< cmp:feature !room -->
...
<!-- <<< cmp:feature room -->
<!-- >>> cmp:feature !room -->
   the Koin graph — DI modules register, `NetworkMonitor` comes online — then
<!-- <<< cmp:feature !room -->
...
<!-- <<< cmp:feature room -->
<!-- >>> cmp:feature !room -->
   matching arm via `ContentStateContainer`.
<!-- <<< cmp:feature !room -->
...
<!-- <<< cmp:feature room -->
<!-- >>> cmp:feature !room -->
`actual`-backed singletons (`NetworkMonitor`) that common modules then depend on
<!-- <<< cmp:feature !room -->
...
<!-- <<< cmp:feature room -->
<!-- >>> cmp:feature !room -->
`actual` per consuming platform — see §4's table (`NetworkMonitor`,
`Modifier.exposeTestTagsForAutomation()`). Convention, not a gate: put the `expect` next to
the domain-shaped contract it serves (`core/` or `data/`, not a grab-bag `platform/` package),
and keep the `actual`'s signature a mechanical mirror — logic
<!-- <<< cmp:feature !room -->
...
<!-- >>> cmp:feature !room -->
This app keeps no on-device database: the example repository
(`data/remote/ItemRepositoryImpl.kt`) returns in-memory sample data, and a screen that needs
data reads it through a repository (§6). The decision not to ship a local store is the
persistence ADR in §8. When you add one, the repository stays the only place that talks to
it — domain never sees storage types; map them to domain models inside `data/`.
<!-- <<< cmp:feature !room -->
<!-- >>> cmp:feature room -->
