...
**The expect/actual boundary** — four shared declarations, each with one `actual` per platform
that needs it:

| Declaration | commonMain (expect) | androidMain | iosMain | desktopMain |
|---|---|---|---|---|
| `NetworkMonitor` | `core/connectivity/NetworkMonitor.kt` | `ConnectivityManager` callback | `nw_path_monitor` (C API) | `NetworkMonitor.desktop.kt` |
| `getDatabaseBuilder()` | `data/local/DatabaseBuilder.kt` | `DatabaseBuilder.android.kt` | `DatabaseBuilder.ios.kt` | `DatabaseBuilder.desktop.kt` |
| `AppDatabaseConstructor` | `data/local/AppDatabase.kt` | Room KSP-generated | Room KSP-generated | Room KSP-generated |
| `Modifier.exposeTestTagsForAutomation()` | `presentation/components/TestTagAutomation.kt` | `testTagsAsResourceId` semantics | same semantics flag (→ `accessibilityIdentifier`) | no-op (no resource-id concept) |

<!-- cmp:generated expect-actual-table -->
_The table above is hand-verified against the tree as of this wave. Wave B
(`qa/lib/arch-doc.mjs`, not yet shipped) replaces it with a real tree walk between these
markers — keep the table in sync by hand until then._
<!-- /cmp:generated -->
...
_File-level inventory (hand-verified as of this wave; Wave B replaces this block with a real
`commonMain` tree walk — the same one the console's Architecture tab uses):_

- `presentation/` — `App.kt`; `components/*.kt` (12 files: `ScreenColumn`, `AppHeader`,
  `ContentStateContainer`, `ContentUiState`, `ListItemCard`, `AppButton`, `AppBottomBar`,
  `BaseScreen`, `EmptyState`, `ErrorState`, `Shimmer`, `TestTagAutomation`); `home/*.kt`
  (the exemplar); `profile/ProfileScreen.kt`; `navigation/*.kt`; `theme/*.kt`.
- `domain/` — `model/` (`Item`, `DomainError`), `repository/ItemRepository.kt`,
  `result/AppResult.kt`, `usecase/GetItemsUseCase.kt`.
- `data/` — `AppResultCatching.kt`; `local/` (`AppDatabase`, `ItemDao`, `DatabaseBuilder` +
  platform actuals); `remote/` (`FirebaseConfig`, `ItemRepositoryImpl`).
- `core/` — `connectivity/NetworkMonitor.kt` (+ actuals); `format/Format.kt`.
- `di/` — `AppModule.kt` (common) + `AndroidModule.kt` / `DesktopModule.kt` (platform).
<!-- /cmp:generated -->
...
_Generated index of `docs/adr/` (hand-maintained until Wave B's walker lands):_
...
_Domain glossary — seeded from the intent interview's vocabulary once
[`specs/intent.md`](../specs/intent.md) is filled in; empty on a fresh scaffold._
<!-- /cmp:generated -->
